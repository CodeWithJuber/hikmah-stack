...
- **A forged append is no longer approved by the next write.** The chain is unkeyed, so anyone who can write the ledger can append a chain-valid record. Before, `verify-ledger` only warned about records past the head file, and the next ordinary write moved the head over them without a word. Now:
  ...
  - `--accept-tail` and `--reset-head` can no longer be combined with `--expect-head`. The pin was ignored and the command exited 0 even when the pin did not match.

  A crash between a record write and its head update now also needs `--accept-tail` once. A reader that sees records past its head snapshot takes a shared lock on the `.lock` sidecar and re-reads the head before reporting them, so verifying during concurrent writes raises no false alarm (stress-tested). If the head moved only part of the way meanwhile, only the records it still does not cover are listed, so a legitimate record is never listed beside a forged one.
