...
- **Configuration notes are no longer refused as credentials.** The generic `KEY=value` rule refused notes such as `TYPESAFE_API_KEY=server-only, never shipped to the client`, `Kubernetes secret: hostlelo-whmcs-creds is mounted into the pod`, and `client_secret=rotated-2026-09 in the vault`, in `remember` and in outbound engine requests. A value is now treated as a description when it is a chain of two or more lowercase words (optionally ending in an ISO year-month or date) or an environment variable name. The vendor token patterns are unchanged. Single words, words plus a bare number (`summer-2024`), mixed case, and random-looking tokens still count as values. Known gap: a passphrase made only of lowercase words (`correct-horse-battery-staple`) is not detected.
