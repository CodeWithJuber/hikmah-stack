...
- **Fewer false blocks on honest completion messages.** A HostLelo review found the rules blocking all six ordinary messages it probed, in both Rust and Python. The rules now leave these alone:
  - `todo`, `tbd` or `fixme` followed by a noun that makes it a name (`the TODO list widget`, `the todo app`, `the TODO comment ... is now handled`). The marker still counts when the same clause says it is open (`one TODO comment is left`), and marker syntax (`TODO:`, `TODO(`) always counts;
  ...
  - any of these right after an opening quote (`the badge reads "Coming soon"`);
  - a first-person promise whose clause leaves it to the user (`If you want, I'll ...`, `Once you approve, we will ...`, `I'll push it when you're ready`, `let me know`). The condition cannot reach past a neighbouring promise or a `;`, so `If you want, I'll update the changelog, and I'll run the e2e suite later` still blocks.

  Deferred work with no user condition still blocks (`I'll push the branch later`, `Once it's merged I will test it on mobile`, `We will run the e2e suite tomorrow`). All six review messages, the review's two true positives, and 21 more allow and block cases are shared golden cases (66 in total). A Rust/Python parity fuzz over 48,000 generated messages found no disagreement; it did find a panic on a non-ASCII letter just before `placeholder` in the new code, fixed before release and kept as a golden case. `docs/EVIDENCE.md` notes that the harness-bench rules numbers predate this change.
