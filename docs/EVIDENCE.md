...
- The rules were narrowed after this measurement (2026-09-23). A term that names a thing (`the TODO list widget`, `a Coming soon badge`, `the search input placeholder`) or a promise left to the user (`If you want, I'll ...`) no longer blocks. The "Rules alone" row, and the rules floor under the shipped v2 row, were measured with the earlier rules and have not been re-run. The change can only lower the rules' block rate on these messages, apart from rare punctuation between `placeholder` and a UI word (`placeholder: text` now blocks). The opt-in engine lift (`HIKMAH_HOOK_ENGINE_LIFT`) was not measured at all.
