...
- `trace.rs` defines `Trace`, which is the unit of memory. A trace is immutable. It carries a kind, provenance (source, authority, locator, and `verified`), confidence, salience, `PrivacyClass`, an optional deadline, an optional claim, and an optional `supersedes` link. Sources starting with `model:` are model-authored. Such traces can never be verified, cannot supersede, stay out of default recall, and are not consolidation evidence. `source` and `verified` are otherwise caller claims. `principal.rs` detects an AI agent session from host env vars (`CLAUDECODE`, `CLAUDE_CODE_*`, `CODEX_*`, `CURSOR_*`, ...), and the CLI then stamps the locator `agent-session:<host>:<id>` and refuses `--verified`, `verify-ledger --accept-tail`, and `verify-ledger --reset-head` (a person's decisions; the refusal must never name a way around the check). `Trace::validate` refuses a verified trace with that locator. CLI tests strip those variables with `common::without_agent_session`, because the suite often runs under an agent. A `prediction` trace is either an engine answer (`model:` source) or a forecast a person or agent recorded with `hikmah predict` (any other source). No prediction can be verified, whatever its source, and its record's `engine` must equal its source without the `model:` prefix.
- `ledger.rs` defines `MemoryStore`, which is an append-only JSONL file with a hash chain.
  ...
  - **Writers:** writers take an exclusive lock on the `<store>.lock` sidecar, never on the ledger itself, because Windows locks are mandatory and would block readers. Under the lock, a writer re-reads records other processes appended, repairs a torn tail with `set_len`, and checks the `<store>.head` file so a write can't paper over a truncation, a rewrite, or records appended without a head update (those need `verify-ledger --accept-tail`). A reader that sees records past its head snapshot waits for in-flight writes under a shared lock before calling them unacknowledged, and lists only the records the head still does not cover, so `verify-ledger` never raises a false alarm during a write. The ledger is opened read+write rather than append-only, because a Windows append handle can't truncate. Every write seeks to the end explicitly.
...
`hook.rs` is the Stop hook. It is a narrow completion-claim check with whole-word matching, negation handling, and code spans skipped. A term that names a thing is not a marker (`the TODO list widget`, `a Coming soon badge`, `the search input placeholder`, a quoted term) unless its clause says it is still open; `TODO comment`/`TODO item` count as open work unless the clause says they were dealt with, and `TODO:` counts even in quotes. A promise gated by an idiom asking for the user's go-ahead (`If you want, I'll ...`, `I'll push it once you approve`) in its own or the preceding comma segment is an offer; a condition about product behaviour (`when you visit /old`) is not. `HIKMAH_HOOK_ENGINE=jev` adds an engine screen:
...
- **The rules are a hard floor by default.** A rules block stands and the engine can only add blocks, unless `HIKMAH_HOOK_ENGINE_LIFT=<p>` (`GateSettings::lift`) is set: then an admitted engine answer with `p < lift` lifts a rules block. Nothing lifts an engine block, and no measurement backs any lift value.
