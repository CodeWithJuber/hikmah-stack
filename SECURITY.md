...
The hash chain covers the exact bytes of every record, so an edited, injected, reordered, or deleted record is detected. Truncation of the tail and a wholesale re-chain are detected only through the `<store>.head` file or a head hash pinned outside the writer's reach (`hikmah verify-ledger --expect-head`). Anyone who can rewrite both the ledger and its head can rewrite history. The chain does not encrypt content and is not a substitute for access control.
