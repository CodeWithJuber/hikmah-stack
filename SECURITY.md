...
The hash chain covers the exact bytes of every record, so a record edited, injected, reordered, or deleted inside the chain is detected. The chain is **not keyed**: anyone who can write the ledger file can compute valid hashes. On its own it therefore detects truncation and rewrites, not appends. Truncation of the tail and a wholesale re-chain are detected only through the `<store>.head` file or a head hash pinned outside the writer's reach (`hikmah verify-ledger --expect-head`).

Records that follow the head file without a head update are treated as unacknowledged. They come from a direct append by another program, an older binary, or a crash between the record write and the head update. `hikmah verify-ledger` fails and lists them under `unacknowledged`, and writes are refused until a person inspects them and runs `hikmah verify-ledger --accept-tail`. The next ordinary write therefore can no longer approve a forged append silently. This is defense in depth, not authentication: the head file sits next to the ledger with the same permissions, so anyone who can also update or delete it can still append or re-chain undetected. Writes are also refused while the ledger and its head file disagree, so an ordinary write cannot hide a truncation. `hikmah verify-ledger --reset-head` accepts the current state after a deliberate repair. The chain does not encrypt content and is not a substitute for access control.
...
`purge` is a tombstone: the content leaves recall but stays in the append-only ledger. Model-authored traces (`model:` sources) cannot be marked verified, cannot supersede other traces, and cannot resolve predictions. `--source` and `--verified` are otherwise what the caller claims. Inside a detected AI agent session (`CLAUDECODE`, `CLAUDE_CODE_*`, `CODEX_*`, `CURSOR_*`, `GEMINI_CLI`, `AI_AGENT`), the CLI stamps each `remember` and `outcome` with the locator `agent-session:<host>:<id>` and refuses `--verified`, and the kernel refuses any verified trace carrying that locator. This stops an agent from verifying its own memory by default. It is not authentication: detection reads environment variables, and a process that clears them passes as a person. Real verification needs authenticated principals, such as a signed `hikmah attest` with a key the agent cannot read, which this reference store does not implement.
